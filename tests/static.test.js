...
test('P0 discovery publication requires explicit admin review',()=>{assert.match(idx,/api\/admin\/ingest\/review/);assert.match(idx,/editorApproved\s*:\s*true/);assert.match(fs.readFileSync(new URL('../src/core/pipeline.js',import.meta.url),'utf8'),/discovery'&&!editorApproved/)});
...
test('P1 collected HTML entities are decoded before publication',()=>assert.match(fs.readFileSync(new URL('../src/core/pipeline.js',import.meta.url),'utf8'),/decodeHtmlEntities\(detail\?\.title\|\|row\.original_title\)/));
test('P0 reviewed draft remains subject to the fact gate',()=>assert.match(fs.readFileSync(new URL('../src/core/pipeline.js',import.meta.url),'utf8'),/const fact=factGate\(record,generated\); if\(!fact\.pass\)/));
test('P0 discovery items bypass the processing queue and remain reviewable',()=>{assert.match(idx,/prepareDiscoveryReview/);assert.match(idx,/usage_policy\s*===\s*["']discovery["']/);assert.match(idx,/api\/admin\/ingests\/prepare/)});
test('P1 canonical is absolute',()=>assert.equal(/canonicalUrl\s*=\s*root\s*\+\s*routeFor/.test(fs.readFileSync(new URL('../src/render/pages.js',import.meta.url),'utf8')),true));
...
test('P0 UGC always enters human moderation',()=>{assert.match(idx,/ugcModerationGate/);assert.match(idx,/status='held'/);assert.match(idx,/api\/admin\/submissions\/review/)});
test('P0 advertising is test-only with zero charged',()=>{const page=fs.readFileSync(new URL('../public/pubblicita.html',import.meta.url),'utf8');const migration=fs.readFileSync(new URL('../migrations/0003_safe_ugc_ads_test.sql',import.meta.url),'utf8');assert.match(idx,/amount_charged_cents: 0/);assert.match(page,/Pagamento simulato/);assert.match(page,/documento non fiscale/);assert.match(migration,/test_no_payment/)});
