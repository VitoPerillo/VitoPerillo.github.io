...
final class YNS_Benessere_V053 {
  ...
  static function foot(){
    ...
    @media(max-width:560px){
      ...
      .ynsba-float__panel{position:fixed;left:8px;right:8px;bottom:54px;width:auto;max-height:56dvh;border-radius:16px}
    ...
    <div class="ynsba-float" data-ynsba-float><button type="button" class="ynsba-float__trigger" aria-expanded="false">Posso aiutarti?</button><div class="ynsba-float__panel" hidden><div class="ynsba-float__top"><span class="ynsba-float__brand">Yoganostress</span><button type="button" class="ynsba-float__mini" data-ynsba-mini>Riduci</button><button type="button" class="ynsba-float__close" data-ynsba-close aria-label="Chiudi assistente" onclick="var p=this.closest('.ynsba-float__panel'),w=this.closest('.ynsba-float'),t=w?w.querySelector('.ynsba-float__trigger'):null;if(p)p.hidden=true;if(w)w.classList.remove('is-minimized');if(t){t.setAttribute('aria-expanded','false');t.focus();}return false;">×</button></div><?php echo self::box(); ?></div></div>
    <script>
