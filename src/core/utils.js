...
export const decodeHtmlEntities = s => String(s||'').replace(/&#(x?[0-9a-f]+);|&(quot|apos|amp|nbsp|agrave|egrave|igrave|ograve|ugrave|aacute|eacute|iacute|oacute|uacute|ldquo|rdquo|lsquo|rsquo);/gi,(m,num,name)=>{if(num){const n=num[0].toLowerCase()==='x'?parseInt(num.slice(1),16):parseInt(num,10);return Number.isFinite(n)?String.fromCodePoint(n):m;}return ({quot:'"',apos:"'",amp:'&',nbsp:' ',agrave:'à',egrave:'è',igrave:'ì',ograve:'ò',ugrave:'ù',aacute:'á',eacute:'é',iacute:'í',oacute:'ó',uacute:'ú',ldquo:'“',rdquo:'”',lsquo:'‘',rsquo:'’'})[name.toLowerCase()]||m;});
